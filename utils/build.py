...
def my_inference_on_dataset(model, data_loader, evaluator, eval_teacher=False):
    ...
    with inference_context(model), torch.no_grad():
        for idx, inputs in enumerate(data_loader):
            ...
            # print(len(outputs))         ## 1
            # print(outputs)              ## [{'instances': Instances(num_instances=1, image_height=512, image_width=700, fields=[pred_boxes: Boxes(tensor([[336.7029, 314.9535, 400.7067, 378.7999]], device='cuda:0')), scores: tensor([0.9993], device='cuda:0'), pred_classes: tensor([0], device='cuda:0')])}]
            # print('--------------------------------------')
...
def build_distillator_optimizer(cfg, network):
    ...
    stu_params = _get_params([network.student, network.adapter], solver_stu.BASE_LR, solver_stu.WEIGHT_DECAY)
    tea_params = _get_params([network.teacher], solver_tea.BASE_LR, solver_tea.WEIGHT_DECAY)
...
def build_distillator_configs(cfg):
    ...
    cfg.MODEL.DISTILLATOR.DISTILLER = 'cwd'
    cfg.MODEL.DISTILLATOR.TEACHER.VIP_SEGMENT_DIM = 8
    cfg.MODEL.DISTILLATOR.LOSS_KL_T = 1
