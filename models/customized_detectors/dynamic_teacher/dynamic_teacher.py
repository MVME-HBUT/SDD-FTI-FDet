...
from ...customized_modules_PHL import WeightedPermuteMLP   
...
class DynamicTeacher(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        self.label_encoder_ = LabelEncoder(
                    ...
                    nr_fpn_channels=self.nr_fpn_channels,
        ...
        self.local_inst_proj_2D = nn.Conv2d(self.nr_fpn_channels, self.nr_fpn_channels, 1, 1) #3,1,1
        ...
        self.refinement_module = nn.Sequential(
                nn.Conv2d(self.nr_fpn_channels, self.nr_fpn_channels, 1, 1),
        ...
        self.segment_dim = cfg.MODEL.DISTILLATOR.TEACHER.VIP_SEGMENT_DIM
        split = self.nr_fpn_channels // self.segment_dim
        
        WeightedPermuteMLP_p2 = WeightedPermuteMLP(dim=self.nr_fpn_channels, dim_h= 178*split, dim_w= 130*split,segment_dim=self.segment_dim).to(device)  
        WeightedPermuteMLP_p3 = WeightedPermuteMLP(dim=self.nr_fpn_channels, dim_h= 90*split, dim_w= 66*split,segment_dim=self.segment_dim).to(device)  
        WeightedPermuteMLP_p4 = WeightedPermuteMLP(dim=self.nr_fpn_channels, dim_h= 46*split, dim_w= 34*split,segment_dim=self.segment_dim).to(device)  
        WeightedPermuteMLP_p5 = WeightedPermuteMLP(dim=self.nr_fpn_channels, dim_h= 24*split, dim_w= 18*split,segment_dim=self.segment_dim).to(device)  
        WeightedPermuteMLP_p6 = WeightedPermuteMLP(dim=self.nr_fpn_channels, dim_h= 12*split, dim_w= 9*split,segment_dim=self.segment_dim).to(device)

        self.attn = [
            WeightedPermuteMLP_p2,
            WeightedPermuteMLP_p3,
            WeightedPermuteMLP_p4,
            WeightedPermuteMLP_p5,
            WeightedPermuteMLP_p6]
    ...
    def forward(self, info_list):
        ...
        for i,feat in enumerate(interact_tea_feats.items()):
            interact_tea_feats['{}'.format(feat[0])] = self.attn[i](feat[1])
