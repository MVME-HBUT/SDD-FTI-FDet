...
class WeightedPermuteMLP_h(nn.Module):
    ...
    def forward(self, x):
        ...
        h = x.reshape(B, self.segment_dim, S, W, H).permute(0, 1, 3, 2, 4).reshape(B, self.segment_dim, W, H*S)
        h = self.mlp_h(h).reshape(B, self.segment_dim, W, H, S).permute(0, 1, 4, 2, 3).reshape(B, C, W, H)
        # print('h.size()',h.size())
        return h

class WeightedPermuteMLP_w(nn.Module):
    def __init__(self, dim, segment_dim=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.segment_dim = segment_dim
    ...
    def forward(self, x):
        ...
        w = x.reshape(B, self.segment_dim, S, W, H).permute(0, 1, 4, 3, 2).reshape(B, self.segment_dim, H, W*S)
        w = self.mlp_w(w).reshape(B, self.segment_dim,  H, W, S).permute(0, 1, 4, 3, 2).reshape(B, C, W, H)
        
        # print('w.size()',w.size())
        return w
    
class WeightedPermuteMLP_c(nn.Module):
    def __init__(self, dim, segment_dim=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.segment_dim = segment_dim
...
class WeightedPermuteMLP(nn.Module):
    def __init__(self, dim, dim_h, dim_w, segment_dim=8, qkv_bias=False, qk_scale=None, attn_drop=0., proj_drop=0.):
        super().__init__()
        self.segment_dim = segment_dim
        ...
        self.attn_h = WeightedPermuteMLP_h(dim_h, self.segment_dim)
        self.attn_w = WeightedPermuteMLP_w(dim_w, self.segment_dim)
        self.attn_c = WeightedPermuteMLP_c(dim_c, self.segment_dim)
